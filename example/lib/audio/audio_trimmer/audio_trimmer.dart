import 'package:example/audio/audio_trimmer/fixed_viewer/fixed_trim_viewer.dart';
    ...
    widget.trimmer.eventStream.listen((event) async {
      if (event == TrimmerEvent.initialized) {
        ...
        final trimAreaDuration = Duration(
            milliseconds:
                (maxAudioLength.inMilliseconds + ((paddingFraction * maxAudioLength.inMilliseconds) * 2).toInt()));

        final shouldScroll =
            trimAreaDuration <= totalDuration && maxAudioLength.compareTo(const Duration(milliseconds: 0)) != 0;
